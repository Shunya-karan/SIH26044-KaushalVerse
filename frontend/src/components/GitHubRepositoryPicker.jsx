...
import { Check, GitBranch, Lock, Search, ExternalLink } from 'lucide-react';
...
export default function GitHubRepositoryPicker({ open, onOpenChange, skill, onSelect }) {
  ...
  const repositories = useMemo(() => {
    ...
    return q ? DEMO_REPOSITORIES.filter((repo) => `${repo.name} ${repo.description} ${repo.language}`.toLowerCase().includes(q)) : DEMO_REPOSITORIES;
  }, [query]);

  const connect = () => setConnected(true);
  const close = () => { setQuery(''); setSelected(null); onOpenChange(false); };
  const confirm = () => {
    if (!selected) return;
    onSelect({ type: 'GitHub Project', value: selected.name, repository: selected, account: DEMO_ACCOUNT });
    close();
  };
    ...
    <Dialog open={open} onOpenChange={(value) => value ? onOpenChange(true) : close()}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Sign in with the student's GitHub account, then select a repository that demonstrates <b>{skill}</b>. Students cannot paste an arbitrary repository URL here.
          </DialogDescription>
        </DialogHeader>
        ...
        {!connected ? (
          <div className="rounded-xl border bg-muted/30 p-6 text-center">
            ...
            <h3 className="mt-4 font-semibold">Connect your GitHub account</h3>
            <p className="mx-auto mt-1 max-w-sm text-sm text-muted-foreground">Authenticate first. After connection, KaushalVerse will show repositories available to this account.</p>
            <Button className="mt-5" onClick={connect}><GitBranch className="mr-2 h-4 w-4" /> Continue with GitHub</Button>
            <p className="mt-3 text-[11px] text-muted-foreground">Demo environment: OAuth is simulated. Production deployment should connect this action to GitHub OAuth.</p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/15 text-sm font-semibold">{DEMO_ACCOUNT.avatar}</div>
                <div><p className="text-sm font-medium">{DEMO_ACCOUNT.name}</p><p className="text-xs text-muted-foreground">@{DEMO_ACCOUNT.login}</p></div>
              </div>
              <Badge variant="success"><Check className="mr-1 h-3 w-3" /> Connected</Badge>
            </div>

            <div className="flex items-center justify-between gap-3">
              <div><p className="text-sm font-medium">Select a repository</p><p className="text-xs text-muted-foreground">Choose from repositories owned by this GitHub account.</p></div>
            ...
            <div className="max-h-64 space-y-2 overflow-y-auto pr-1">
              {repositories.map((repo) => {
                ...
                return <button key={repo.id} type="button" onClick={() => setSelected(repo)} className={`w-full rounded-lg border p-3 text-left transition ${isSelected ? 'border-primary bg-primary/5 ring-1 ring-primary/20' : 'hover:bg-muted/50'}`}>
                  <div className="flex items-start gap-3"><GitBranch className="mt-0.5 h-4 w-4 shrink-0" /><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><p className="truncate text-sm font-medium">{repo.name}</p>{isSelected && <Check className="h-4 w-4 shrink-0 text-primary" />}</div><p className="mt-1 text-xs text-muted-foreground">{repo.description}</p><div className="mt-2 flex flex-wrap items-center gap-2"><Badge variant="outline">{repo.language}</Badge><span className="text-[11px] text-muted-foreground">Updated {repo.updated}</span>{repo.private && <Lock className="h-3 w-3 text-muted-foreground" />}</div></div></div>
                </button>;
              })}
